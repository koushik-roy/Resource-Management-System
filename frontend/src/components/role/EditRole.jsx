...
function Roles() {
  ...
  const getdata = async () => {
    await axios
      .get(`http://localhost:9191/role/roleById/${params.id}`)
      .then((res) => {
  ...
  const onSubmit = (data) => {
    console.log(role);
    roleService.submitEditRole(role);
  };
        ...
        <div className="group" style={{ margin: "1% auto" }}>
          ...
          >
            <div className="heading" style={{ marginTop: "33px" }}>
              <h1 className="text">Create Role</h1>
            </div>
            <form
              name="myForm"
              className="form"
